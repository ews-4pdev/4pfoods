...
class StripeHelper {
  ...
  function __construct($today = NULL) {
    ...
    $this->_today = ($today && (ENVIRONMENT != 'production')) ? $today : date('Y-m-d');
  ...
  function createDeliveries() {
    ...
    //  We are looking for orders whose delivery day is
    //  X days from now. Find weekday and date for matching
    $matchDay = date('D', strtotime($this->_today.' +'.$daysBeforeNow.' Days'));
    $matchDate = date('Y-m-d', strtotime($this->_today.' +'.$daysBeforeNow.' Days'));
  ...
  function createOrders($date = null) {
    ...
    //  X days from now. Find weekday and date for matching
    $matchDay = ( new DateTime($this->_today) )->modify('+'.$daysBeforeNow.' day')->format('D');
    $matchDate = ( new DateTime($this->_today) )->modify('+'.$daysBeforeNow.' day')->format('Y-m-d');
  ...
  function executeCharges(){
    ...
    $beginDate = date('Y-m-d', strtotime($this->_today.' -6 day'));
      $this->log("Payment - Orders from ".$beginDate." to ".$this->_today."\n");
    ...
    $aOrders = OrderQuery::create()
      ...
      ->filterByDeliveryScheduledFor(array('min' => $beginDate, 'max' => $this->_today))
      ->filterByPaidAt(NULL)
      ->find();
      $this->log("Payment - Total order for payments = ".$aOrders->count()."\n");
    ...
    foreach ($aCharges as $charge) {
      ...
      $this->log("Payment - Total Success = ".$success." Total errors = ".$error."\n");
  ...
  function executeChargeForOrders( $aOrders ) {
    ...
    $charge[ 'totalCharge' ] += $charge[ 'Deliverycharge' ];
    $charge['tax'] = round( ( $charge['totalCharge'] * ( $charge['tax'] / 100 ) ), 2 );
    $charge[ 'totalCharge' ] +=  $charge['tax'];
  ...
  function chargeCustomer($data) {
    ...
    try {
      $result = $this->_stripe('Charge', $data);
        ...
        $this->log("Payment - exception = ".$this->_parseError($e)."\n");
      return $this->_parseError($e);
    }
